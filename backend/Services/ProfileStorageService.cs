...
public class ProfileStorageService
{
     ...
     static string _profileStorageDirectory = Path.Combine(_rootDirectory,"profiles");
    ...
    public ProfileStorageService()
    {
        ...
        if(!Directory.Exists(_profileStorageDirectory)) 
        {
            Directory.CreateDirectory(_profileStorageDirectory);
        }
    }
    ...
    public List<Profile> GetAllProfiles()
    {
        ...
        foreach (var file in Directory.GetFiles(_profileStorageDirectory, "*.json"))
        {
    ...
    private string GetFilePath(string id, string name)
    {
        ...
        return Path.Combine(_profileStorageDirectory, sanitizedFileName);
    }

    // Helper: Locate a file by ID (ignores the name in the filename)
    private string GetFilePathById(string id)
    {
        return Directory.GetFiles(_profileStorageDirectory, $"{id}_*.json").FirstOrDefault() ?? string.Empty;
    }
